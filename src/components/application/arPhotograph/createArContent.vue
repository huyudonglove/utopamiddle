<template>
  <div>
    ...
    <el-form label-position="right" label-width="150px" style="width: 80%">
      ...
      <el-form-item label="关联素材">
        <div>
          ...
          <el-row class="tac" style="border-bottom:1px solid #eeeeee; padding-bottom:15px;">
            <el-input placeholder="请输入内容" class="input-with-select" v-model="inputX" style="width: 205px;" maxlength="50">
          ...
          <el-table ref="multipleTable" :data="table" tooltip-effect="dark" :cell-style="cellStyle" @selection-change="selectionChange">
            <el-table-column type="selection" width="50" :selectable="(row)=>row.state==1"></el-table-column>
            <el-table-column prop="id" label="ID" width="50"></el-table-column>
            <el-table-column :label="`${tableTitle.name}名称`" prop="name"></el-table-column>
            <el-table-column label="所属类别" prop="materialCategoryName"></el-table-column>
            <el-table-column :label="tableTitle.display" prop="name">
              <template slot-scope="scope">
                ...
                <span style="cursor: pointer;" v-if="tableTitle.name!='歌曲'&&tableTitle.name!='视频'">
                  <img :src="scope.row.imgUrl?`/static/${scope.row.imgUrl}`:''" alt=""  style="width:150px;height:150px" v-focus>
                </span>
            ...
            <el-table-column v-if="tableTitle.lyric" :label="tableTitle.lyric" prop="isLyric">
              <template slot-scope="scope">
                <span v-if="scope.row.isLyric==1">是</span>
                <span v-if="scope.row.isLyric==0">否</span>
              </template>
            </el-table-column>
            <el-table-column v-if="tableTitle.package" :label="'安卓'+tableTitle.package+'(M)'" prop="androidResourcePackageSize" width="200">
              <template slot-scope="scope">
                {{scope.row.androidResourcePackageSize?((scope.row.androidResourcePackageSize/(1024*1024))+'').slice(0,4):''}}
              </template>
            </el-table-column>
            <el-table-column v-if="tableTitle.package" :label="'IOS'+tableTitle.package+'(M)'" prop="iosResourcePackageSize" width="200">
              <template slot-scope="scope">
                {{scope.row.iosResourcePackageSize?((scope.row.iosResourcePackageSize/(1024*1024))+'').slice(0,4):''}}
              </template>
            </el-table-column>
            <el-table-column v-if="tableTitle.time" :label="tableTitle.time" prop="durationTimeStr" width="200"></el-table-column>
      ...
      <el-form-item>
        <el-button v-if="isCreate" type="primary" @click="addOrEdit">立即添加</el-button>
        <el-button v-if="isEdit" type="primary" @click="addOrEdit">立即修改</el-button>
        <el-button @click="$router.push({path:`/application/assetsList/content/2/${$route.params.appId}`,query:JSON.parse($route.query.oldQuery)});">取消</el-button>
      </el-form-item>
    </el-form>
  </div>
</template>
  ...
  data(){
    return{
      ...
      tableTitleList:[
        ...
        {name:'模型',display:'模型展示',module:'102'},
        {name:'模型',display:'模型展示',module:'103'},
    ...
    addOrEdit(){
      ...
      if(this.isCreate){
        createArContent(this.allParams).then(res=>{
          this.$router.push({path:`/application/assetsList/content/2/${this.$route.params.appId}`,query:JSON.parse(this.$route.query.oldQuery)});
          this.reload();
        })
      }else if(this.isEdit){
